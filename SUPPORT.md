# State of the Races Support

State of the Races provides campaign-finance, voter-registration, candidate, and historical-result information for Oregon's 2026 statewide and legislative elections.

## Getting help

- Pull down on the candidate list to request the latest available data.
- Confirm that your iPhone has an internet connection if the latest refresh is unavailable.
- If ORESTAR is temporarily unavailable, the app preserves the most recent successful figures rather than replacing them with zero.
- To report a problem, request a correction, or suggest an improvement, [open a support request](https://github.com/prestonmann1991/beyond-the-ballot/issues/new).

## Data source and disclaimer

Campaign-finance figures are derived from public records in the Oregon Secretary of State's ORESTAR system. The ORESTAR `Balance Deficit` field is displayed as reported and is not necessarily the same as cash on hand.

State of the Races is an independent informational app. It is not affiliated with, endorsed by, or operated by the Oregon Secretary of State, any candidate, or any campaign.

[Read the privacy policy](PRIVACY.md)
